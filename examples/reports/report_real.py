...
def report_real(region):
    while True:
        ...
        try:
            ...
            # add the factors
            factor1 = VolumeUpMa250Factor(region=region, start_timestamp='2018-01-01', end_timestamp=target_date)
            ...
            if long_stocks:
                # use block to filter
                block_selector = BlockSelector(region=region, start_timestamp='2020-01-01', long_threshold=0.8)
                block_selector.run()
                long_blocks = block_selector.get_open_long_targets(timestamp=target_date)

                if long_blocks:
                    blocks: List[Block] = Block.query_data(region=region, provider=Provider.Sina, entity_ids=long_blocks,
                                                           return_type='domain')

                    info = [f'{block.name}({block.code})' for block in blocks]
                    msg = ' '.join(info) + '\n'

                    block_stocks: List[BlockStock] = BlockStock.query_data(region=region,
