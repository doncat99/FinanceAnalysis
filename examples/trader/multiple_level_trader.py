...
class MultipleLevelBullTrader(StockTrader):
    def init_selectors(self, entity_ids, entity_schema, exchanges, codes, start_timestamp, end_timestamp):
        ...
        # 最近20周黄白线在0轴上
        week_bull_factor = KeepBullFactor(self.region, entity_ids=entity_ids, entity_schema=entity_schema, exchanges=exchanges,
                                          codes=codes, start_timestamp=start_timestamp, end_timestamp=end_timestamp,
        ...
        day_bull_factor = KeepBullFactor(self.region, entity_ids=entity_ids, entity_schema=entity_schema, exchanges=exchanges,
                                         codes=codes, start_timestamp=start_timestamp, end_timestamp=end_timestamp,
                                         provider=Provider.JoinQuant, level=IntervalLevel.LEVEL_1DAY)
    ...
    def filter_selector_long_targets(self, timestamp, selector: TargetSelector, long_targets: List[str]) -> List[str]:
        ...
        if selector.level == IntervalLevel.LEVEL_1DAY:
            ...
            df = FinanceFactor.query_data(self.region, entity_ids=long_targets, order=FinanceFactor.op_income_growth_yoy.desc(),
...
if __name__ == '__main__':
    trader = MultipleLevelBullTrader(Region.CHN, start_timestamp='2019-01-01', end_timestamp='2020-01-01')
    trader.run()
