...
class MaVolTrader(StockTrader):
    ...
    def on_trading_signals(self, trading_signals: List[TradingSignal]):
        ...
        # 发送最近20天的交易信号
        if target_date + datetime.timedelta(20) > now_pd_timestamp(self.region):
            ...
            if positions:
                ...
                msg = msg + '目前持仓: ' + entity_ids_to_msg(self.region, current_stocks) + '\n'
            ...
            if long_stocks:
                msg = msg + '买入: ' + entity_ids_to_msg(self.region, long_stocks) + '\n'

            if short_stocks:
                msg = msg + '卖出: ' + entity_ids_to_msg(self.region, short_stocks) + '\n'
