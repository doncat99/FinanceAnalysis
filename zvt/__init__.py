...
# import ray
# ray.init(object_store_memory=10**9, logging_level=logging.FATAL)
...
def init_log(file_name='analysis.log', log_dir=None, simple_formatter=True):
    ...
    root_logger.setLevel(logging.DEBUG if zvt_config['debug'] else logging.INFO)

    file_name = os.path.join(log_dir, file_name)

    file_log_handler = RotatingFileHandler(file_name, maxBytes=524288000, backupCount=10)

    file_log_handler.setLevel(logging.DEBUG if zvt_config['debug'] else logging.INFO)
    ...
    if simple_formatter:
        formatter = logging.Formatter(
            "%(asctime)s  %(levelname)-7s  %(processName)-11s  %(message)s")
    else:
        formatter = logging.Formatter(
