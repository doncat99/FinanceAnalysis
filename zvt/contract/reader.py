...
class DataReader(object):
    ...
    def __init__(self,
        ...
        if entity_schema and not self.entity_ids:
            df = get_entities(self.region, entity_schema=entity_schema, provider=self.entity_provider,
    ...
    def load_window_df(self, provider: Provider, data_schema, window):
        ...
        for entity_id in self.entity_ids:
            df = data_schema.query_data(self.region, 
                                        provider=provider,
    ...
    def load_data(self):
        ...
        self.data_df = self.data_schema.query_data(self.region,
    ...
    def move_on(self, to_timestamp: Union[str, pd.Timestamp] = None,
        ...
        while True:
            for entity_id, df in self.data_df.groupby(level=0):
                ...
                added_df = self.data_schema.query_data(self.region,
                                                       provider=self.provider,
                                                       columns=self.columns,
