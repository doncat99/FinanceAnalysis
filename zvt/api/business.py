...
def get_traders() -> List[str]:
    df = get_group(provider=Provider.ZVT, data_schema=AccountStats, column=AccountStats.trader_name, group_func=None)
...
def get_order_securities(trader_name):
    items = get_db_session(provider=Provider.ZVT, data_schema=Order).query(Order.entity_id).filter(
