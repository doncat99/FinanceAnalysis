...
def get_etf_stocks(region: Region, timestamp, code=None, codes=None, ids=None, provider: Provider=Provider.Default):
    latests: List[EtfStock] = EtfStock.query_data(region, provider=provider, code=code, 
    ...
    if latests:
        ...
        # 获取最新的报表
        df = EtfStock.query_data(region, provider=provider, code=code, codes=codes, ids=ids, 
                                 end_timestamp=timestamp,
        ...
        # 季报，需要结合 年报或半年报 来算持仓
            ...
            while True:
                ...
                pre_df = EtfStock.query_data(region, provider=provider, code=code, codes=codes, 
...
def get_kdata(region, entity_id=None, entity_ids=None, level=IntervalLevel.LEVEL_1DAY.value, 
    ...
    return data_schema.query_data(region, entity_ids=entity_ids, level=level, provider=provider,
